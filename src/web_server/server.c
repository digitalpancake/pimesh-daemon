...
void* handle_client(void* arg) {
    ...
    ssize_t bytes_received =
        recv(client_file_descriptor, buffer, BUFFER_SIZE, 0);
...
void run_server() {
    ...
    // Create a socket on IP v4, with TCP and the default protocol for the
    // socket type
    ...
    socket_address.sin_addr.s_addr =
        INADDR_ANY; // Accepts connections from any server interface
    socket_address.sin_port =
        htons(PORT); // Set the port (in network byte order)
